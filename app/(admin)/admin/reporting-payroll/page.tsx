...
const ReportingPayroll = () => {
  ...
  return (
    <div className="space-y-8 px-4 py-2">
      ...
      {/* Reporting Options removed for brevity, check techstahr-main if needed */}
        ...
        <DialogContent className="max-w-md">
          ...
          {!chargeResponse ? (
            <div className="space-y-4">
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => { setPayEmployeeDialogOpen(false); setChargeResponse(null); }}>Cancel</Button>
                <Button className="bg-[#4CAF50] text-white hover:bg-[#45A049]" onClick={handleCreateCharge} disabled={isCreatingCharge || Number(paymentAmount) < 200 || !paymentAmount}>{isCreatingCharge ? "Creating Charge..." : "Create Charge"}</Button>
              </DialogFooter>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="rounded-lg border bg-green-50 p-4">
                <p className="text-sm font-medium text-green-800 mb-2">Charge created successfully!</p>
                <p className="text-xs text-green-700">An OTP has been sent to your email. Click "Next" to complete the payment process.</p>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => { setPayEmployeeDialogOpen(false); setChargeResponse(null); queryClient.invalidateQueries({ queryKey: ["payroll-list"] }); }}>Close</Button>
                <Button className="bg-[#4CAF50] text-white hover:bg-[#45A049]" onClick={handleCompletePayment} disabled={!chargeResponse?.next_action?.redirect_url?.url}><ExternalLink className="mr-2 h-4 w-4" />Next - Complete Payment</Button>
              </DialogFooter>
            </div>
          )}
        </DialogContent>
