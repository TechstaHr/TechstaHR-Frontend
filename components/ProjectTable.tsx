...
import { clearTrackingSession, saveTrackingSession } from "@/lib/utils/tracking-session";
...
export default function ProjectTable() {
  ...
  // Initialize clocked in projects from server data
  useEffect(() => {
    if (userProjects?.projects) {
      const clockedInFromServer = new Set<string>();
      userProjects.projects.forEach((project: Project) => {
        if (project.isClockedIn) {
          clockedInFromServer.add(project._id);
        }
      });
  ...
  // Alternative: Use localStorage as fallback if server doesn't provide clock status
  useEffect(() => {
    // Load from localStorage on component mount
    const savedClockStatus = localStorage.getItem("clockedInProjects");
    if (savedClockStatus) {
      try {
        const parsed = JSON.parse(savedClockStatus);
        if (Array.isArray(parsed)) {
          setClockedInProjects(new Set(parsed));
        }
      } catch (error) {
        console.error("Error parsing saved clock status:", error);
        localStorage.removeItem("clockedInProjects");
      }
    }
  }, []);

  // Save to localStorage whenever clockedInProjects changes
  useEffect(() => {
    localStorage.setItem(
      "clockedInProjects",
      JSON.stringify([...clockedInProjects]),
    );
  }, [clockedInProjects]);
